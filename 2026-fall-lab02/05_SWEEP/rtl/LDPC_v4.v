...
//   h_q[u*42 +: 42]          : head (level 0) of CNU u, {sgn[6:0], mag[34:0]}
...
module LDPC_DP (
    ...
    // Head <- level 1 decompressed one cycle ahead: port p gets min2n if p == idx,
    // else min1n; keeps the idx decode out of the CNU loop
    generate
        for(gu = 0; gu < 16; gu = gu + 1) begin : HEAD_GEN
            ...
            for(gp = 0; gp < 7; gp = gp + 1) begin : HEAD_PORT
                assign h_d[gu*42 + gp*5 +: 5] = (w[8:6] == gp)? w[13:9] : w[18:14];
            end
            assign h_d[gu*42 + 35 +: 7] = {^w[5:0], w[5:0]};
        end
    endgenerate
...
module LDPC_CNU (
    ...
    generate
        for(gp = 0; gp < 7; gp = gp + 1) begin : PORT_GEN
            ...
            // Old C2V operand: conditional inversion, ~s_o is the carry-in
    ...
    // Normalization: round_half_up(0.75 * m) with m = y + s -> (3y + (s ? 5 : 2)) >> 2
