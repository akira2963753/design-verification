...
//                              in ones' complement (value + carry-in bit [p*6 + 5])
...
module LDPC_DP (
    ...
    // Head <- level 1 decompressed one cycle ahead into -C2V_old per port, ones'
    // complement v = {~s, mag ^ {5{~s}}}: -C2V_old = sext(v) + v[5] (|C2V| <= 23).
    // Magnitude min2n on the idx port, else min1n; the idx decode and the sign
    // handling stay out of the CNU loop, the +1 becomes the adder carry-in.
    generate
        for(gu = 0; gu < 16; gu = gu + 1) begin : HEAD_GEN
            ...
            for(gp = 0; gp < 7; gp = gp + 1) begin : HEAD_PORT
                ...
                assign mag = (w[8:6] == gp)? w[13:9] : w[18:14];
                // negative C2V: +mag (carry 0), positive C2V: ~mag (carry 1)
                assign h_d[gu*42 + gp*6 +: 6] = {~sgn[gp], mag ^ {5{~sgn[gp]}}};
            end
        end
    endgenerate
...
//=============================================================
// Per port p (-C2V_old comes from the head in ones' complement, x - c2v = x + sext(v) + v[5]):
...
module LDPC_CNU (
    ...
    generate
        for(gp = 0; gp < 7; gp = gp + 1) begin : PORT_GEN
            ...
            // -C2V_old in ones' complement, sign extended to 8b; carry-in = sign bit
            assign neg_o = {{2{old_neg[gp*6 + 5]}}, old_neg[gp*6 +: 6]};

            // V2C = A - C2V_old, magnitude key with clip at 31
            assign ta = a_in[gp*8 +: 8] + neg_o + {7'd0, neg_o[7]};
            assign y = ta ^ {8{ta[7]}};
            ...
            assign tb = b_in[gp*8 +: 8] + neg_o + {7'd0, neg_o[7]};
