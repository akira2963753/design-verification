...
// Registers: B bank (APP) 1024 FF, A bank 1024 FF, C2V FIFO 912 FF, head 384 FF,
...
//   h_q[u*24 +: 24]          : head (level 0) of CNU u, {sgn[6:0], hot[6:0], min2n, min1n}
...
module LDPC_DP (
    ...
    // Head <- level 1 with idx decoded to one-hot one cycle ahead (the idx decode stays
    // out of the CNU loop, only a 2:1 mux per port remains in the CNU)
    generate
...
module LDPC_CNU (
    ...
    generate
        for(gp = 0; gp < 7; gp = gp + 1) begin : PORT_GEN
            ...
            // Old C2V operand: min2n on the one-hot idx port, conditional inversion,
            // ~s_o is the carry-in
