...
module LDPC_DP (
    ...
    //=============================================================
    // Per slot:
    //   B: shift chain (IO) or the write-back rotation of the current layer
    //   A: flooding layer 0~2 hold the snapshot (move with the layout), IO follows the
    //      chain, otherwise the write-back; the late write-back value passes one 2:1
    //      mux for both banks
    wire [7:0] tail_in;
    wire a_hold, a_early_sel;

    assign tail_in = (in_data_valid)? {{2{in_data[5]}}, in_data} : 8'd0;
    assign a_hold = !mode && dec && (layer != 2'd3);
    assign a_early_sel = a_hold || shift_en;

    generate
        for(gg = 0; gg < 8; gg = gg + 1) begin : B_GRP
            for(gr = 0; gr < 16; gr = gr + 1) begin : B_SLOT
                wire [7:0] chain, a_early;
                wire [31:0] wb, mv;
                ...
                // Flooding moves of layer 0~2; layer 3 is never held (duplicate of layer 2)
                for(gk = 0; gk < 3; gk = gk + 1) begin : MV_GEN
                    assign mv[gk*8 +: 8] = a_q[(fn_src(gk, gg)*16 + (gr + fn_rot(gk, gg)) % 16)*8 +: 8];
                end
                assign mv[31:24] = mv[23:16];

                assign b_d[(gg*16 + gr)*8 +: 8] = (shift_en)? chain : wb[layer*8 +: 8];
                assign a_early = (a_hold)? mv[layer*8 +: 8] : chain;
                assign a_d[(gg*16 + gr)*8 +: 8] = (a_early_sel)? a_early : wb[layer*8 +: 8];
            end
        end
    endgenerate
