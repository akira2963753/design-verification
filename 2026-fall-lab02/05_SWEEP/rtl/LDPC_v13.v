...
// Outputs are registered (reg -> output), latency = 4 x iterations + 1.
// Registers: B bank (APP) 1024 FF, A bank 1024 FF, C2V FIFO 912 FF, head 672 FF,
// control 17 FF, output 10 FF (FSM state and outputs are reset).
...
module LDPC (
    ...
    //                        Interconnect
    //=============================================================
    wire mode, dec, shift_en, out_now, pass;
    wire [1:0] layer;
...
// The first output is formed in the syndrome check cycle (out_now) and leaves the
// output registers one cycle later, so latency = 4 x iterations + 1.
module LDPC_CTRL (
    input clk,
    input rst_n,
    input in_mode_valid,
...
module LDPC_CNU (
    ...
    //                          CN Update
    //=============================================================
    // k1 / k2: minimum and second minimum key of |V2C|;
