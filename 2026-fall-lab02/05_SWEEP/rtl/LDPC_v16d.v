...
// Registers: B bank (APP) 1024 FF, A bank 896 FF (7b), C2V FIFO 912 FF, head 672 FF,
...
//   a_q[s*7 +: 7]            : A bank, V2C source (layered: B, flooding: snapshot), saturated
//                              to +-63: |C2V| <= 23 keeps the clipped V2C exact
...
module LDPC_DP (
    ...
    generate
        for(gu = 0; gu < 16; gu = gu + 1) begin : CNU_GEN
            for(gp = 0; gp < 7; gp = gp + 1) begin : PORT_GEN
                assign cnu_a[gu*56 + gp*8 +: 8] = {a_q[(fn_pgrp(gp)*16 + gu)*7 + 6], a_q[(fn_pgrp(gp)*16 + gu)*7 +: 7]};
    ...
    // otherwise (layered, flooding layer 3, IO) A follows B_next saturated to 7b
    wire a_hold;
        ...
        for(gg = 0; gg < 8; gg = gg + 1) begin : A_GRP
            for(gr = 0; gr < 16; gr = gr + 1) begin : A_SLOT
                ...
                for(gk = 0; gk < 3; gk = gk + 1) begin : MV_GEN
                    assign mv[gk*7 +: 7] = a_q[(fn_src(gk, gg)*16 + (gr + fn_rot(gk, gg)) % 16)*7 +: 7];
                end

                // B_next saturated to +-63: |APP| >= 64 still gives |V2C| > 31 with the same sign
                assign bn = b_d[(gg*16 + gr)*8 +: 8];
                assign bs = (bn[7] ^ bn[6])? {bn[7], {6{!bn[7]}}} : bn[6:0];
                // layer 3 is never held (don't care)
                assign mv[27:21] = bs;

                assign a_d[(gg*16 + gr)*7 +: 7] = (a_hold)? mv[layer*7 +: 7] : bs;
            end
        end
    endgenerate
