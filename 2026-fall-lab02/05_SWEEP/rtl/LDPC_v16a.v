...
module LDPC_CNU (
    ...
    wire [4:0] min1n, min2n, n1_a, n2_a, n1_b, n2_b;
    wire [3:0] h_a;
    wire [2:0] h_b;
    wire [2:0] idx;
    wire c_f, c_x, c_y, total;
    ...
    generate
        for(gp = 0; gp < 7; gp = gp + 1) begin : PORT_GEN
            ...
            // C2V_new magnitude straight from the merge candidates (one-hot AND-OR, no global
            // min2 select): port 0~3 gets n1_b when B holds min1, min2 (n2_a / n1_b) on the A min1
            // port, n1_a otherwise; port 4~6 mirrored
            if(gp < 4) begin : SEL_A
                assign s1 = c_f || (h_a[gp] && !c_y);
                assign s2 = !c_f && h_a[gp] && c_y;
                assign s3 = !c_f && !h_a[gp];
                assign mag_n = ({5{s1}} & n1_b) | ({5{s2}} & n2_a) | ({5{s3}} & n1_a);
            end
            else begin : SEL_B
                assign s1 = !c_f || (h_b[gp - 4] && !c_x);
                assign s2 = c_f && h_b[gp - 4] && c_x;
                assign s3 = c_f && !h_b[gp - 4];
                assign mag_n = ({5{s1}} & n1_a) | ({5{s2}} & n2_b) | ({5{s3}} & n1_b);
            end

            // B_new = (B - C2V_old) + C2V_new
    ...
    //                          CN Update
    //=============================================================
    // min1n / min2n / idx: C2V word; merge candidates and controls for the port select
    LDPC_MIN7 u_min7 (
        .key(key),
        .min1n(min1n),
        .min2n(min2n),
