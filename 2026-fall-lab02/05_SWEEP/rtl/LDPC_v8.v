...
//=============================================================
//   7-input normalized min1 / min2 finder (21 comparators)
//=============================================================
// All pairs are compared in parallel: port n is min1 when no port beats it and min2
// when exactly one port beats it (ties: the lower port wins). The 7 keys are
// normalized in parallel with the compares, the one-hot ranks select normalized values.
module LDPC_MIN7 (
    input [41:0] key,
    output [4:0] min1n,
    output [4:0] min2n,
    ...
    generate
        // Normalized keys
        for(gi = 0; gi < 7; gi = gi + 1) begin : NORM_GEN
            LDPC_NORM u_norm (
                .key(key[gi*6 +: 6]),
                .mag(nk[gi*5 +: 5])
            );
        ...
        // 21 comparators: port j (j > i) beats port i only when strictly smaller
        for(gi = 0; gi < 7; gi = gi + 1) begin : CMP_I
            for(gj = gi + 1; gj < 7; gj = gj + 1) begin : CMP_J
                wire w;

                assign w = (key[gj*6 +: 6] < key[gi*6 +: 6]);
                assign bt[gi*7 + gj] = w;
                assign bt[gj*7 + gi] = !w;
            end
            assign bt[gi*7 + gi] = 1'b0;
        end

        // Rank of port n from the 7 "beaten by" bits, split 3 + 4:
        // rank 0 = none set, rank 1 = exactly one set
        for(gi = 0; gi < 7; gi = gi + 1) begin : RANK_GEN
            wire [6:0] b;
            wire z_lo, z_hi, e_lo, e_hi;

            assign b = bt[gi*7 +: 7];
            assign z_lo = ~|b[2:0];
            assign z_hi = ~|b[6:3];
            assign e_lo = (^b[2:0]) & ~(&b[2:0]);
            assign e_hi = ((b[3] ^ b[4]) & ~(b[5] | b[6])) | ((b[5] ^ b[6]) & ~(b[3] | b[4]));
            assign r0[gi] = z_lo & z_hi;
            assign r1[gi] = (e_lo & z_hi) | (z_lo & e_hi);
        end
    endgenerate

    // One-hot AND-OR select of the normalized values
    assign min1n = ({5{r0[0]}} & nk[4:0]) | ({5{r0[1]}} & nk[9:5]) | ({5{r0[2]}} & nk[14:10]) | ({5{r0[3]}} & nk[19:15])
        | ({5{r0[4]}} & nk[24:20]) | ({5{r0[5]}} & nk[29:25]) | ({5{r0[6]}} & nk[34:30]);
    assign min2n = ({5{r1[0]}} & nk[4:0]) | ({5{r1[1]}} & nk[9:5]) | ({5{r1[2]}} & nk[14:10]) | ({5{r1[3]}} & nk[19:15])
        | ({5{r1[4]}} & nk[24:20]) | ({5{r1[5]}} & nk[29:25]) | ({5{r1[6]}} & nk[34:30]);
    assign hot = r0;
    assign idx = {r0[4] | r0[5] | r0[6], r0[2] | r0[3] | r0[6], r0[1] | r0[3] | r0[5]};
