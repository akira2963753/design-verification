...
// A bank update. Outputs are registered (reg -> output), latency = 8 x iterations + 1.
// Registers: B bank (APP) 1024 FF, A bank 1024 FF, C2V FIFO 912 FF, head 672 FF,
// CNU pipeline 608 FF, control 18 FF, output 10 FF (FSM state and outputs are reset).
...
module LDPC (
    ...
    //                        Interconnect
    //=============================================================
    wire mode, dec, ph, shift_en, out_now, pass;
    wire [1:0] layer;
...
// ph 1: CNU stage 2 and write-back), OUT: 128 APP outputs. The first output is formed
// in the syndrome check cycle (out_now) and leaves the output registers one cycle later,
// so latency = 8 x iterations + 1.
module LDPC_CTRL (
    input clk,
    input rst_n,
    input in_mode_valid,
    ...
    wire idle, out_en, io_last, chk, done, warn_now;
    ...
    // out_now / warn_now: output of this cycle, before the output registers
    assign out_now = out_en || done;
    assign warn_now = (done)? !pass : (out_en && warn_q);
    ...
    assign shift_en = idle || out_now;

    //=============================================================
    //                             FSM
    //=============================================================
    // Reset only on the FSM state and the output registers: outputs must be 0 while
    // rst_n is low (clock stopped)
    ...
    //=============================================================
    // reg -> output
    always @(posedge clk or negedge rst_n) begin : OUT_SEQ
        if(!rst_n) begin
            out_valid <= 1'b0;
            out_warn <= 1'b0;
        end
...
module LDPC_DP (
    ...
    //                           Output
    //=============================================================
    // Output register (reg -> output): VN 0 of column 0 is the head of the shift chain
    always @(posedge clk or negedge rst_n) begin : OUT_SEQ
        if(!rst_n) out_data <= 8'd0;
        else out_data <= (out_now)? b_q[HEAD*8 +: 8] : 8'd0;
    end
