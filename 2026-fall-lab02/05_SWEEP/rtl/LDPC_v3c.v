...
//   fifo[l][u] : C2V word, 20b, l = 1~3     -> f_q[((l-1)*16+u)*20 +: 20]
//   head[u]    : level 0, decompressed old C2V of port 0~6 {sgn[6:0], mag[34:0]}
//                -> h_q[u*42 +: 42], loaded with decompress(level 1)
...
module LDPC_DP (
    ...
    generate
        for(gu = 0; gu < 16; gu = gu + 1) begin : CNU_GEN
            ...
            LDPC_CNU u_cnu (
                .a_in(cnu_a[gu*56 +: 56]),
                .b_in(cnu_b[gu*56 +: 56]),
                .old_mag(h_q[gu*42 +: 35]),
                .old_sgn(h_q[gu*42 + 35 +: 7]),
    ...
    assign f_d = {((dec)? cnu_wn : 320'd0), f_q[959:320]};

    // Level 0: decompress level 1 one cycle ahead (removes idx decode from the CNU loop)
    generate
        for(gu = 0; gu < 16; gu = gu + 1) begin : HEAD_GEN
            wire [19:0] w;
            assign w = f_q[gu*20 +: 20];
            for(gp = 0; gp < 7; gp = gp + 1) begin : HEAD_PORT
                assign h_d[gu*42 + gp*5 +: 5] = (w[9:7] == gp)? w[14:10] : w[19:15];
            end
            assign h_d[gu*42 + 35 +: 7] = w[6:0];
        end
    endgenerate
...
module LDPC_CNU (
    ...
    generate
        for(gp = 0; gp < 7; gp = gp + 1) begin : PORT_GEN
            ...
            // Old C2V (already decompressed), sign-magnitude operand: x - c2v_old = x + op_o + ~s_o
            assign mag_o = old_mag[gp*5 +: 5];
