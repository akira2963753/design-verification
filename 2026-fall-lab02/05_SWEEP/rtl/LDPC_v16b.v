...
module LDPC_CNU (
    ...
    generate
        for(gp = 0; gp < 7; gp = gp + 1) begin : PORT_GEN
            ...
            // raw key for the compares, clipped key for the selected values
            assign raw[gp*6 +: 6] = {ta[4:0] ^ {5{ta[7]}}, ta[7]};
            assign v_in[gp] = inr;
...
module LDPC_MIN4 (
    ...
    // k_j < k_i from the raw keys q = {ones' complement magnitude, sign}, the in-range flags v
    // merged after the compare: an out-of-range key is 62, raw key 63 (V2C -32) is above 62
    function lt;
        input [5:0] qj;
        input [5:0] qi;
        input vj;
        input vi;
        begin
            lt = (vi)? ((vj)? (qj < qi) : (&qi)) : (vj && !(&qj[5:1]));
        end
    endfunction

    assign w01 = lt(q1, q0, v[1], v[0]);
    assign w02 = lt(q2, q0, v[2], v[0]);
    assign w03 = lt(q3, q0, v[3], v[0]);
    assign w12 = lt(q2, q1, v[2], v[1]);
    assign w13 = lt(q3, q1, v[3], v[1]);
    assign w23 = lt(q3, q2, v[3], v[2]);
